...
render_site_map <- function(traits, render_date, legend_title, image_path) {
  ...
  if(!file.exists(image_path)){
    ...
    map <- addLayersControl(map, overlayGroups = "Trait values",
                            position = "topright")
    
    fullfield_image <- stack(image_path)
    
    overlay_map <- viewRGB(x = fullfield_image, map = map, layer.name = 'full-field image')
